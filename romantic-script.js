...
document.addEventListener('DOMContentLoaded', function() {
    ...
    saveMemoryBtn.addEventListener('click', function() {
        ...
        saveMemoryBtn.style.background = 'linear-gradient(135deg, #a8e6cf, #6bcf9c)';
        ...
        setTimeout(() => {
            ...
            saveMemoryBtn.style.background = 'linear-gradient(135deg, #ff9eb5, #ff6b8b)';
        }, 2000);
    ...
    futureButton.addEventListener('click', function() {
        ...
        for (let i = 0; i < 15; i++) {
            createStarParticle(
                futureButton.getBoundingClientRect().left + futureButton.offsetWidth/2,
                futureButton.getBoundingClientRect().top + futureButton.offsetHeight/2
            );
        }
    ...
    function createConfetti() {
        ...
        for (let i = 0; i < confettiCount; i++) {
            ...
            confetti.animate([
                ...
                { opacity: 0, transform: `translateY(100vh) rotate(${Math.random() * 720}deg)` }
            ], {
    ...
    function createFuturePopup() {
        ...
        saveFutureBtn.addEventListener('click', function() {
           ...
           localStorage.setItem('nextDate', next5th.getTime());

            
            // Update button
            saveFutureBtn.innerHTML = '<i class="fas fa-check"></i> Date Marked!';
            saveFutureBtn.style.background = 'linear-gradient(135deg, #a8e6cf, #6bcf9c)';
    ...
    function startCountdown() {
        // Set target to next 5th of month
        const now = new Date();
        let targetDate;
        
        // Check if we have a saved date
        const savedDate = localStorage.getItem('nextDate');
        if (savedDate) {
            targetDate = new Date(parseInt(savedDate));

        } else {
            // Default to next month's 5th
            let nextMonth = now.getMonth() + 1;
            let year = now.getFullYear();
        ...
        observer.observe(futurePopup, { attributes: true, attributeFilter: ['class'] });
    }
    ...
    const decorativeElements = document.querySelectorAll('.heart-icon, .button-heart, .heart-emoji');
